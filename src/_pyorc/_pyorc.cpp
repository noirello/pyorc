...
PYBIND11_MODULE(_pyorc, m) {
    ...
    py::class_<Stripe>(m, "stripe")
        ...
        .def_property_readonly("writer_timezone", [](Stripe &s) { return s.writerTimezone(); })
        .def_readonly("current_row", &Stripe::currentRow)
        .def_readonly("row_offset", &Stripe::firstRowOfStripe);
    py::class_<Reader>(m, "reader")
        .def(py::init<py::object, uint64_t, std::list<uint64_t>, std::list<std::string>>(),
            py::arg("fileo"),
