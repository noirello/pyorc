...
PyORCInputStream::PyORCInputStream(py::object fp) {
    if (!(py::hasattr(fp, "read") && py::hasattr(fp, "seek"))) {
        throw py::type_error("Parameter must be a file-like object, but `" + (std::string)(py::repr(fp)) + "` provided");
    }
    pyread = fp.attr("read");
    pyseek = fp.attr("seek");
...
uint64_t PyORCInputStream::getLength() const {
    return totalLength;
}

uint64_t PyORCInputStream::getNaturalReadSize() const {
    return 128 * 1024;
}

const std::string& PyORCInputStream::getName() const {
    return filename;
}


void PyORCInputStream::read(void* buf, uint64_t length, uint64_t offset) {
    char *src;
...
uint64_t PyORCOutputStream::getLength() const {
    return bytesWritten;
}

uint64_t PyORCOutputStream::getNaturalWriteSize() const {
    return 128 * 1024;
}

const std::string& PyORCOutputStream::getName() const {
    return filename;
}

void PyORCOutputStream::write(const void* buf, size_t length) {
    if (closed) {
